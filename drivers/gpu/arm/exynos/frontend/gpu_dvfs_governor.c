...
{
    /* New governor logic with proportional jumps and min residency */

    int max_lev = gpex_clock_get_table_idx(gpex_clock_get_max_clock());
    int min_lev = gpex_clock_get_table_idx(gpex_clock_get_min_clock());
    int cur_lev = dvfs->step;
    int target_lev = cur_lev;
    int hs_level = gpex_clock_get_table_idx(dvfs->interactive.highspeed_clock);
    bool can_change = true;

    /* Prevent too frequent frequency switches by enforcing a minimum residency time */
    if (time_before(jiffies, dvfs->interactive.last_jiffy + dvfs->interactive.min_residency_ticks))
        can_change = false;

    /* Highspeed boost logic (preserved from original interactive governor) */
    if (can_change &&
        (cur_lev > max_lev) &&
        (utilization > dvfs->table[cur_lev].max_threshold)) {
        if ((hs_level > 0) && (cur_lev > hs_level) &&
            (utilization > dvfs->interactive.highspeed_load)) {
            if (dvfs->interactive.delay_count >= dvfs->interactive.highspeed_delay) {
                target_lev = hs_level; /* jump directly to highspeed_clock */
                dvfs->interactive.delay_count = 0;
            } else {
    ...
    /* If highspeed not applied, calculate proportional jumps */
    if (can_change && target_lev == cur_lev) {
        if ((cur_lev > max_lev) && (utilization > dvfs->table[cur_lev].max_threshold)) {
            int over = utilization - dvfs->table[cur_lev].max_threshold;
            int steps = dvfs->interactive.up_step_base + (over / 10); /* every +10% load = +1 step */
            if (steps > dvfs->interactive.max_steps_per_decision)
                steps = dvfs->interactive.max_steps_per_decision;
            target_lev = cur_lev - steps;
            if (target_lev < max_lev)
                target_lev = max_lev;
        } else if ((cur_lev < min_lev) && (utilization < dvfs->table[cur_lev].min_threshold)) {
            int under = dvfs->table[cur_lev].min_threshold - utilization;
            int steps = dvfs->interactive.down_step_base + (under / 10);
            if (steps > dvfs->interactive.max_steps_per_decision)
                steps = dvfs->interactive.max_steps_per_decision;

            /* honor down_staycount hysteresis */
            dvfs->down_requirement--;
            if (dvfs->down_requirement <= 0) {
                target_lev = cur_lev + steps;
                if (target_lev > min_lev)
                    target_lev = min_lev;
                dvfs->down_requirement = dvfs->table[target_lev].down_staycount;
            } else {
                dvfs->interactive.delay_count = 0;
            }
        } else {
            /* inside thresholds: reset counters */
            dvfs->interactive.delay_count = 0;
            dvfs->down_requirement = dvfs->table[cur_lev].down_staycount;
        }
    }

    /* Clamp to max_clock_limit if needed */
    if (target_lev != cur_lev) {
        if (dvfs->table[target_lev].clock > gpex_clock_get_max_clock_limit())
            target_lev = gpex_clock_get_table_idx(gpex_clock_get_max_clock_limit());
        dvfs->step = target_lev;
        dvfs->interactive.last_jiffy = jiffies;
        dvfs->down_requirement = dvfs->table[target_lev].down_staycount;
    }
